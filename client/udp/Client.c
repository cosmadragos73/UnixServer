...
int main(int argc, char *argv[])
{
	...
	err_sys("Adresa nu e valida");

    if (sscanf(argv[2], "%" SCNu16, &tport_h)!=1)
	err_sys("Portul nu e valid");
    tport_n = htons(tport_h);

    /* create the socket */
    printf("Creez socket\n");
    s = Socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    printf("done, numarul socket-ului: %d\n",s);
...
{	
    ...
    printf("trimit mesajul ACK catre server ->");
    strcpy(buf, "ACK");
	len = strlen(buf);
	...
	if (n>0)
	{
		...
		{
			rbuf[n] = '\0';
			showAddr("Am primit un raspuns de la:", &from);
			printf(": [%s]\n", rbuf);
		}
		else{
			printf("Eroare la initializare.... Server busy\n");	
			return;		
		}
	}
	else 
	{
		printf(" Nu am primit niciun raspuns dupa %d secunde \n",TIMEOUT);
		return;
	}
    
    while(strcmp(buf,"CLOSE")!=0)
    {
	    mygetline(buf, BUFLEN, " Introdu mesaj(max 127 char, sau scrie 'CLOSE' pentru a inchide conexiunea");
	    len = strlen(buf);
